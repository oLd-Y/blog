---
title: 通过源码学 lua （1）- im-select.lua
categories: 计算机
tags:
  - 通过源码学X
  - neovim
...
本系列通过查看他人的源码，学习常用的 lua 语法和配置规则。

由于最近在配置 neovim，需要一个自动切换中英文输入的工具，发现了 im-select.nvim。 这个项目的核心只有一个文件，比较简单，因此尝试通过对这个文件进行分析来对 neovim 的配置和 lua 进行学习。

im-select.lua 的源码如下所示：
```lua
local M = {}
