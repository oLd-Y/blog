...
weight: 1
draft: false
lastmod: 2025-02-05T17:04:55+08:00
---
## 知识性的书籍总结
...
4. 按照“要点+概述”的格式进行输出。
5. 原文中若包含本章的标题，将其作为 mardown 的二级标题。
...
6. 每当我阅读完毕书中的某个章节，我就会让你总结这章中的要素。
7. 我会提供给你这段原文，请你**根据当前原文**，将文中提到的人物、地点完整地记录下来。

\[DEMAND]

8. 输出格式为 markdown，我给你的原文中有本章的标题，将其作为 mardown 的二级标题。人物、地点这些类别以三级标题给出，而其下的每个项以无序列表的形式列出。
9. 若是出现了对这些人物、地点的描写，你需要将其总结整理到相应的无序列表项后面。对人物，你需要记录整理的有外貌、身份等；对地点，你需要关注它的位置、作用等。
10. 对于只出现了名字的人名、地名，若是已登场过，则记录他们与本章事件的联系；若是未登场过（仅提及了姓名埋一个伏笔），则记录他们的姓名或是称号以及相关的上下文即可。
11. 人物和地点需要加粗，以便更好地进行辨认。
12. 在人物对话中也有可能出现这些小说要素，不允许出现遗漏。
13. 对于一些名著，你的语料库中可能已经知道了他们的身份，但不允许提及本章中未曾说明的内容。
...
14. **通用描述**：使用容易理解的语言来描述每个提示词。避免过于复杂或模糊的表达。
15. **具体的色彩和样式描述**：为每个提示词提供与其相关的颜色、风格或主题特征，例如“古老的城市景观”、“未来世界的建筑”，等等。
16. **尽量简单，但不缺乏细节**：用简洁明了的方式描述要画的元素，同时保持一定的具体性，使得AI绘图工具能更好地理解并实现艺术家想要的效果。
17. **避免模糊或多义性的词汇**：尽量使用特定和清晰的提示，避免可能给模型带来的误解。
18. **保持一致性**：将所有提示用同样的风格和语法书写，以便于机器更好地学习并理解艺术家想要实现的效果。

请根据以上ai绘画中提示词的书写原则，将下列中文描述内容扩写成规范的提示词。注意：

19. 扩写的提示词内容完整，细节丰富。
20. 将生成的中文内容转换成英文格式进行输出。
21. 不要输出扩写英文以外的内容。
