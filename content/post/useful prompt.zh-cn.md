...
weight: 1
draft: false
lastmod: 2025-02-16T17:05:55+08:00
---
## 知识性的书籍总结
...
- 若是出现了对这些人物、地点的描写，你需要将其总结整理到相应的无序列表项后面。对人物，你需要记录整理的有外貌、身份等；对地点，你需要关注它的位置、作用等。
...
2. **通用描述**：使用容易理解的语言来描述每个提示词。避免过于复杂或模糊的表达。
3. **具体的色彩和样式描述**：为每个提示词提供与其相关的颜色、风格或主题特征，例如“古老的城市景观”、“未来世界的建筑”，等等。
4. **尽量简单，但不缺乏细节**：用简洁明了的方式描述要画的元素，同时保持一定的具体性，使得AI绘图工具能更好地理解并实现艺术家想要的效果。
5. **避免模糊或多义性的词汇**：尽量使用特定和清晰的提示，避免可能给模型带来的误解。
6. **保持一致性**：将所有提示用同样的风格和语法书写，以便于机器更好地学习并理解艺术家想要实现的效果。

请根据以上ai绘画中提示词的书写原则，将下列中文描述内容扩写成规范的提示词。注意：

7. 扩写的提示词内容完整，细节丰富。
8. 将生成的中文内容转换成英文格式进行输出。
9. 不要输出扩写英文以外的内容。
